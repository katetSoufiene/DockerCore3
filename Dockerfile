...
COPY . .
WORKDIR "/src/WebApiCore3"
RUN dotnet build "WebApiCore3.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "WebApiCore3.csproj" -c Release -o /app/publish

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "WebApiCore3.dll"]